...
namespace premier_league_genetic_algorithm.BL
{
    public class FitnessCaclulator
    {
        ...
        private List<SoftConstraint> initializeSoftConstraints(IEnumerable<Player> players)
        {
            ...
            var ictConstraint = new IctConstraint(0.4f, (float)minTeamValue(playersIct), (float)maxTeamValue(playersIct));

            var playersPointsPerGame = players.Select(p => p.points_per_game);
            var pointsPerGameConstraint = new PointsPerGameConstraint(0.3f, minTeamValue(playersPointsPerGame), maxTeamValue(playersPointsPerGame));

            var playersTotalPoints = players.Select(p => Convert.ToDouble(p.total_points));
            var totalPointsConstaint = new TotalPointsConstraint(0.3f, Convert.ToInt32(minTeamValue(playersTotalPoints)), Convert.ToInt32(maxTeamValue(playersTotalPoints)));
